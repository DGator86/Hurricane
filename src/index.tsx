...
<body class="bg-gray-900 text-white">
    ...
    <script>
        ...
        function renderWhalesVariants(variants, dataSource, chainSize) {
            ...
            const isGreekVariant = (variant) => ['greek_exposure', 'gex', 'gamma_exposure'].includes(variant);
            ...
            const summarizeVariant = (variant, result) => {
                ...
                let detail;
                let meta = result?.endpoint || (result?.attempts && result.attempts[0]);

                if (success && isGreekVariant(variant)) {
                    const data = result?.data || {};
                    const nested = data?.data || {};
                    const candidates = [
                        pickNested(data, ['net_gex', 'netGEX', 'net', 'net_gamma_exposure']),
                        pickNested(nested, ['net_gex', 'netGEX', 'net', 'net_gamma_exposure'])
                    ];
                    const netValue = candidates.map(parseNumber).find((v) => v !== null && v !== undefined);

                    if (netValue !== null && netValue !== undefined) {
                        const formatted = formatExposure(netValue);
                        detail = (netValue >= 0 ? '+' : '') + formatted + ' net GEX';
                    }

                    const gammaFlipCandidates = [
                        pickNested(data, ['gamma_flip', 'gammaFlip', 'gamma_flip_point']),
                        pickNested(nested, ['gamma_flip', 'gammaFlip', 'gamma_flip_point'])
                    ];
                    const gammaFlip = gammaFlipCandidates.map(parseNumber).find((v) => v !== null && v !== undefined);
                    if (gammaFlip !== null && gammaFlip !== undefined) {
                        const formattedFlip = gammaFlip.toFixed(2);
                        meta = meta ? (meta + ' · γ ' + formattedFlip) : ('γ ' + formattedFlip);
                    }
                }

                if (!detail) {
                    detail = success ? (count > 0 ? count + ' records' : 'connected') : (result?.error || 'no data');
                }
